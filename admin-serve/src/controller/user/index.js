class Users {
  ...
  static async queryUser(userName) {
    ...
    const user = await Users.database.query(sqlStr, [userName])
    return Boolean(user !== false && user.length)
  }

  static async login(userName, passWord) {
    const sqlStr = 'select * from users where userName = ? and passWord = ?;'
    const isUser = await Users.queryUser(userName)
    const userInfo = await Users.database.query(sqlStr, [userName, passWord])
